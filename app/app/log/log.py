...
async def save_request_async(
    request: Request,
    response: Response = None,
    trace_back: str = ""
)-> None:
    clinet_host = request.client.host
    ...
    def get_route_handler(self) -> Callable:
        ...
        async def custom_route_handler(request: Request)-> Response:
            ...
            if not response.background:
                ...
                tasks.add_task(save_request_async, request, response)
                response.background = tasks
            else:
                response.background.add_task(save_request_async, request, response)
                return response
